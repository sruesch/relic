...
static void rsa(void) {
	rsa_t pub, prv;
	uint8_t in[10], new[10], h[MD_LEN], out[BN_BITS_RELIC / 8 + 1];
	int out_len, new_len;
	...
	BENCH_ONCE("cp_rsa_gen", cp_rsa_gen(pub, prv, BN_BITS_RELIC));

	BENCH_BEGIN("cp_rsa_enc") {
		out_len = BN_BITS_RELIC / 8 + 1;
		new_len = out_len;
	...
	BENCH_ONCE("cp_rsa_gen_basic", cp_rsa_gen_basic(pub, prv, BN_BITS_RELIC));
	...
	BENCH_ONCE("cp_rsa_gen_quick", cp_rsa_gen_quick(pub, prv, BN_BITS_RELIC));
...
static void rabin(void) {
	rabin_t pub, prv;
	uint8_t in[1000], new[1000], out[BN_BITS_RELIC / 8 + 1];
	...
	BENCH_ONCE("cp_rabin_gen", cp_rabin_gen(pub, prv, BN_BITS_RELIC));
...
static void benaloh(void) {
	...
	BENCH_ONCE("cp_bdpe_gen", cp_bdpe_gen(pub, prv, bn_get_prime(47), BN_BITS_RELIC));
...
static void paillier(void) {
	...
	BENCH_ONCE("cp_phpe_gen", cp_phpe_gen(n, l, BN_BITS_RELIC / 2));

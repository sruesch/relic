...
static int util(void) {
	...
	TRY {
		...
		TEST_BEGIN("copy and comparison are consistent") {
			bn_rand(a, BN_POS, BN_BITS_RELIC);
			bn_rand(b, BN_POS, BN_BITS_RELIC);
			bn_rand(c, BN_POS, BN_BITS_RELIC);
		...
		TEST_BEGIN("signal test is correct") {
			bn_rand(a, BN_POS, BN_BITS_RELIC);
			bn_rand(b, BN_NEG, BN_BITS_RELIC);
		...
		TEST_BEGIN("reading and writing a negative number are consistent") {
			int len = BN_BYTES_RELIC;
			bn_rand(a, BN_NEG, BN_BITS_RELIC);
			for (int j = 2; j <= 64; j++) {
				bits = bn_size_str(a, j);
				bn_write_str(str, bits, a, j);
				bn_read_str(b, str, bits, j);
...
static int multiplication(void) {
	...
	TRY {
		...
		TEST_BEGIN("multiplication is commutative") {
			bn_rand(a, BN_POS, BN_BITS_RELIC / 2);
			bn_rand(b, BN_POS, BN_BITS_RELIC / 2);
			bn_mul(d, a, b);
			bn_mul(e, b, a);
			TEST_ASSERT(bn_cmp(d, e) == CMP_EQ, end);
		} TEST_END;
		...
		TEST_BEGIN("multiplication is associative") {
			bn_rand(a, BN_POS, BN_BITS_RELIC / 3);
			bn_rand(b, BN_POS, BN_BITS_RELIC / 3);
			bn_rand(c, BN_POS, BN_BITS_RELIC / 3);
			bn_mul(d, a, b);
			bn_mul(d, d, c);
			bn_mul(e, b, c);
			bn_mul(e, a, e);
		...
		TEST_BEGIN("multiplication is distributive") {
			bn_rand(a, BN_POS, BN_BITS_RELIC / 2);
			bn_rand(b, BN_POS, BN_BITS_RELIC / 2);
			bn_rand(c, BN_POS, BN_BITS_RELIC / 2);
			bn_add(d, a, b);
			bn_mul(d, c, d);
			bn_mul(e, c, a);
			bn_mul(f, c, b);
...
static int doubling_halving(void) {
	...
	TRY {
		...
		TEST_BEGIN("doubling is consistent") {
			bn_rand(a, BN_POS, BN_BITS_RELIC - 1);
			bn_add(b, a, a);
			bn_dbl(c, a);
			TEST_ASSERT(bn_cmp(b, c) == CMP_EQ, end);
		} TEST_END;
...
static int shifting(void) {
	...
	TRY {
		...
		TEST_BEGIN("shifting by half digit is consistent") {
			bn_rand(a, BN_POS, BN_BITS_RELIC - BN_DIGIT / 2);
		...
		TEST_BEGIN("shifting by 2 digits is consistent") {
			bn_rand(a, BN_POS, BN_BITS_RELIC - 2 * BN_DIGIT);
		...
		TEST_BEGIN("shifting by 1 digit and half is consistent") {
			bn_rand(a, BN_POS, BN_BITS_RELIC - BN_DIGIT - BN_DIGIT / 2);
...
static int division(void) {
	...
	TRY {
		...
		TEST_BEGIN("trivial negative division is correct") {
			bn_rand(a, BN_NEG, BN_BITS_RELIC / 2);
			bn_rand(b, BN_POS, BN_BITS_RELIC);
			bn_div(e, a, b);
			bn_div_rem(c, d, a, b);
			TEST_ASSERT(bn_cmp(e, c) == CMP_EQ, end);
		...
		TEST_BEGIN("division by negative is correct") {
			bn_rand(a, BN_POS, BN_BITS_RELIC);
			bn_rand(b, BN_NEG, BN_BITS_RELIC / 2);
			bn_div(e, a, b);
			bn_div_rem(c, d, a, b);
			TEST_ASSERT(bn_cmp(e, c) == CMP_EQ, end);
			bn_mul(e, c, b);
...
static int reduction(void) {
	...
	TRY {
		...
		TEST_BEGIN("pseudo-mersenne reduction is correct") {
			bn_rand(a, BN_POS, BN_BITS_RELIC);
			bn_rand(b, BN_POS, BN_BITS_RELIC / 2);
			bn_rand(c, BN_POS, BN_BITS_RELIC / 4);
			if (bn_is_zero(c))
				bn_set_dig(c, 1);
			bn_set_2b(b, BN_BITS_RELIC / 2);
			bn_sub(b, b, c);
			bn_mod(c, a, b);
...
static int symbol(void) {
	...
	TRY {
		...
		TEST_BEGIN("jacobi symbol is a homomorphism") {
			bn_rand(a, BN_POS, BN_BITS_RELIC / 2);
			bn_rand(b, BN_POS, BN_BITS_RELIC / 2);
			bn_rand(p, BN_POS, BN_BITS_RELIC / 2);
			if (bn_is_even(p)) {
				bn_add_dig(p, p, 1);
			}
			bn_mul(c, a, b);
		...
		TEST_BEGIN("jacobi symbol respects quadratic reciprocity") {
			...
			bn_rand(q, BN_POS, BN_BITS_RELIC / 2);
			if (bn_is_even(q)) {
				bn_add_dig(q, q, 1);
			}
...
static int prime(void) {
	...
	TRY {
		...
		TEST_ONCE("basic prime generation is consistent") {
			bn_gen_prime_basic(p, BN_BITS_RELIC);
			TEST_ASSERT(bn_is_prime(p) == 1, end);
		} TEST_END;
#endif
		...
		TEST_ONCE("safe prime generation is consistent") {
			bn_gen_prime_safep(p, BN_BITS_RELIC);
			TEST_ASSERT(bn_is_prime(p) == 1, end);
			bn_sub_dig(p, p, 1);
			bn_hlv(p, p);
			TEST_ASSERT(bn_is_prime(p) == 1, end);
		...
		TEST_ONCE("strong prime generation is consistent") {
			bn_gen_prime_stron(p, BN_BITS_RELIC);
			TEST_ASSERT(bn_is_prime(p) == 1, end);
		} TEST_END;
#endif
...
static int factor(void) {
	...
	TRY {
		...
		TEST_ONCE("integer factorization is consistent") {
			bn_gen_prime(p, 16);
			bn_rand(n, BN_POS, BN_BITS_RELIC - 16);
			bn_mul(n, n, p);
			if (bn_factor(q, n) == 1) {
...
static int recoding(void) {
	...
	int w, k, l;
	uint8_t d[BN_BITS_RELIC + 1];
	signed char e[2 * (BN_BITS_RELIC + 1)];
	...
	TRY {
		...
		TEST_BEGIN("regular recoding is correct") {
			for (w = 2; w <= 8; w++) {
				...
				l = BN_BITS_RELIC + 1;
				bn_rec_reg(e, &l, a, BN_BITS_RELIC, w);
				bn_zero(b);
				for (k = l - 1; k >= 0; k--) {
					bn_lsh(b, b, w - 1);
					if (e[k] > 0) {

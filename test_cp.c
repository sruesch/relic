...
static int rsa(void) {
	int code = STS_ERR;
	rsa_t pub, prv;
	uint8_t in[10], out[BN_BITS_RELIC / 8 + 1], h[MD_LEN];
	int il, ol;
	int result;
	...
	TRY {
		...
		result = cp_rsa_gen_basic(pub, prv, BN_BITS_RELIC);
		...
		result = cp_rsa_gen_quick(pub, prv, BN_BITS_RELIC);
...
static int rabin(void) {
	...
	TRY {
		...
		result = cp_rabin_gen(pub, prv, BN_BITS_RELIC);
...
static int benaloh(void) {
	...
	TRY {
		...
		result = cp_bdpe_gen(pub, prv, bn_get_prime(47), BN_BITS_RELIC);

		TEST_BEGIN("benaloh encryption/decryption is correct") {
			TEST_ASSERT(result == STS_OK, end);
			len = BN_BITS_RELIC / 8 + 1;
			rand_bytes(buf, 1);
...
static int paillier(void) {
	int code = STS_ERR;
	bn_t a, b, c, d, n, l, s;
	uint8_t in[BN_BITS_RELIC / 8 + 1], out[BN_BITS_RELIC / 8 + 1];
	int in_len, out_len;
	int result;
	...
	TRY {
		...
		result = cp_phpe_gen(n, l, BN_BITS_RELIC / 2);

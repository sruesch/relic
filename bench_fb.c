...
static void arith(void) {
	fb_t a, b, c, d[2], t[FB_TABLE_RELIC_MAX];
	dv_t e;
	bn_t f;
	int bits;
	...
	fb_null(d[0]);
	fb_null(d[1]);
	for (int i = 0; i < FB_TABLE_RELIC_MAX; i++) {
		fb_null(t[i]);
	}
	dv_null(e);
	bn_null(f);
...
#if FB_SQR == TABLE_RELIC || !defined(STRIP)
	...
	for (int i = 0; i < FB_TABLE_RELIC; i++) {
		fb_new(t[i]);
	}
	...
	for (int i = 0; i < FB_TABLE_RELIC_QUICK; i++) {
		fb_new(t[i]);
	}

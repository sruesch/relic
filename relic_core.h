...
typedef struct _ctx_t {
	...
	fb_st fb_tab_sqr[MAX_TERMS][FB_TABLE_RELIC];
	/** Pointers to the elements in the tables of repeated squarings. */
	fb_st *fb_tab_ptr[MAX_TERMS][FB_TABLE_RELIC];
#endif /* FB_INV == ITOHT */
#endif /* WITH_FB */
	...
	eb_st eb_pre[EB_TABLE_RELIC];
	/** Array of pointers to the precomputation table. */
	eb_st *eb_ptr[EB_TABLE_RELIC];
#endif /* EB_PRECO */
#endif /* WITH_EB */
...
#ifdef EP_PRECO
	/** Precomputation table for generator multiplication. */
	ep_st ep_pre[EP_TABLE_RELIC];
	/** Array of pointers to the precomputation table. */
	ep_st *ep_ptr[EP_TABLE_RELIC];
#endif /* EP_PRECO */
#endif /* WITH_EP */
...
#ifdef EP_PRECO
	/** Precomputation table for generator multiplication.*/
	ep2_st ep2_pre[EP_TABLE_RELIC];
	/** Array of pointers to the precomputation table. */
	ep2_st *ep2_ptr[EP_TABLE_RELIC];
#endif /* EP_PRECO */
#if ALLOC == STACK
/** In case of stack allocation, we need to get global memory for the table. */
	fp2_st _ep2_pre[3 * EP_TABLE_RELIC];
#endif /* ALLOC == STACK */
#endif /* WITH_EPX */

...
static int fixed(void) {
	int code = STS_ERR;
	bn_t n, k;
	ep2_t p, q, r, t[EPX_TABLE_RELIC_MAX];
	...
	for (int i = 0; i < EPX_TABLE_RELIC_MAX; i++) {
		ep2_null(t[i]);
	}
	...
	TRY {
		...
		for (int i = 0; i < EPX_TABLE_RELIC; i++) {
			ep2_new(t[i]);
		}
		...
		for (int i = 0; i < EPX_TABLE_RELIC_BASIC; i++) {
			ep2_new(t[i]);
		}
		...
		for (int i = 0; i < EPX_TABLE_RELIC_YAOWI; i++) {
			ep2_new(t[i]);
		}
		...
		for (int i = 0; i < EPX_TABLE_RELIC_NAFWI; i++) {
			ep2_new(t[i]);
		}
		...
		for (int i = 0; i < EPX_TABLE_RELIC_COMBS; i++) {
			ep2_new(t[i]);
		}
		...
		for (int i = 0; i < EPX_TABLE_RELIC_COMBD; i++) {
			ep2_new(t[i]);
		}
		...
		for (int i = 0; i < EPX_TABLE_RELIC_LWNAF; i++) {
			ep2_new(t[i]);
		}

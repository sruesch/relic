 ...
 * fixed precision.
 */
#define BN_BITS_RELIC 	((int)BN_PRECI)
 ...
 */
#define BN_DIGS		((int)((BN_BITS_RELIC)/(BN_DIGIT) + (BN_BITS_RELIC % BN_DIGIT > 0)))

/**
 * Size in bytes of a block sufficient to store the required precision.
 */
#define BN_BYTES_RELIC 	((int)((BN_BITS_RELIC)/8 + ((BN_BITS_RELIC % 8) > 0)))

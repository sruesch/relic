...
static int fixed(void) {
	int code = STS_ERR;
	bn_t n, k;
	ep_t p, q, r, t[EP_TABLE_RELIC_MAX];
	...
	for (int i = 0; i < EP_TABLE_RELIC_MAX; i++) {
		ep_null(t[i]);
	}
	...
	TRY {
		...
		for (int i = 0; i < EP_TABLE_RELIC; i++) {
			ep_new(t[i]);
		}
		...
		for (int i = 0; i < EP_TABLE_RELIC_BASIC; i++) {
			ep_new(t[i]);
		}
		...
		for (int i = 0; i < EP_TABLE_RELIC_YAOWI; i++) {
			ep_new(t[i]);
		}
		...
		for (int i = 0; i < EP_TABLE_RELIC_NAFWI; i++) {
			ep_new(t[i]);
		}
		...
		for (int i = 0; i < EP_TABLE_RELIC_COMBS; i++) {
			ep_new(t[i]);
		}
		...
		for (int i = 0; i < EP_TABLE_RELIC_COMBD; i++) {
			ep_new(t[i]);
		}
		...
		for (int i = 0; i < EP_TABLE_RELIC_LWNAF; i++) {
			ep_new(t[i]);
		}

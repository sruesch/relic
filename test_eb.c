...
static int fixed(void) {
	int code = STS_ERR;
	bn_t n, k;
	eb_t p, q, r, t[EB_TABLE_RELIC_MAX];
	...
	for (int i = 0; i < EB_TABLE_RELIC_MAX; i++) {
		eb_null(t[i]);
	}
	...
	TRY {
		...
		for (int i = 0; i < EB_TABLE_RELIC; i++) {
			eb_new(t[i]);
		}
		...
		for (int i = 0; i < EB_TABLE_RELIC_BASIC; i++) {
			eb_new(t[i]);
		}
		...
		for (int i = 0; i < EB_TABLE_RELIC_YAOWI; i++) {
			eb_new(t[i]);
		}
		...
		for (int i = 0; i < EB_TABLE_RELIC_NAFWI; i++) {
			eb_new(t[i]);
		}
		...
		for (int i = 0; i < EB_TABLE_RELIC_COMBS; i++) {
			eb_new(t[i]);
		}
		...
		for (int i = 0; i < EB_TABLE_RELIC_COMBD; i++) {
			eb_new(t[i]);
		}
		...
		for (int i = 0; i < EB_TABLE_RELIC_LWNAF; i++) {
			eb_new(t[i]);
		}

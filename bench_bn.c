...
static void util(void) {
	dig_t digit;
	char str[BN_BYTES_RELIC * 3 + 1];
	uint8_t bin[BN_BYTES_RELIC];
	dig_t raw[BN_DIGS];
	bn_t a, b;
	...
	bn_rand(b, BN_POS, BN_BITS_RELIC);

	BENCH_BEGIN("bn_copy") {
		bn_rand(a, BN_POS, BN_BITS_RELIC);
	...
	BENCH_BEGIN("bn_get_bit") {
		bn_rand(a, BN_POS, BN_BITS_RELIC);
		BENCH_ADD(bn_get_bit(a, BN_BITS_RELIC / 2));
	}
	BENCH_END;

	BENCH_BEGIN("bn_set_bit") {
		bn_rand(a, BN_POS, BN_BITS_RELIC);
		BENCH_ADD(bn_set_bit(a, BN_BITS_RELIC / 2, 1));
	}
	BENCH_END;
	...
	BENCH_BEGIN("bn_set_2b") {
		bn_rand(a, BN_POS, BN_BITS_RELIC);
		BENCH_ADD(bn_set_2b(a, BN_BITS_RELIC / 2));
	}
	BENCH_END;

	BENCH_BEGIN("bn_rand") {
		BENCH_ADD(bn_rand(a, BN_POS, BN_BITS_RELIC));
	}
	BENCH_END;
...
static void arith(void) {
	...
	BENCH_BEGIN("bn_lsh") {
		bn_rand(a, BN_POS, BN_BITS_RELIC);
		BENCH_ADD(bn_lsh(c, a, BN_BITS_RELIC / 2 + BN_DIGIT / 2));
	}
	BENCH_END;

	BENCH_BEGIN("bn_rsh") {
		bn_rand(a, BN_POS, BN_BITS_RELIC);
		BENCH_ADD(bn_rsh(c, a, BN_BITS_RELIC / 2 + BN_DIGIT / 2));
	}
	BENCH_END;

	BENCH_BEGIN("bn_div") {
		bn_rand(a, BN_POS, 2 * BN_BITS_RELIC - BN_DIGIT / 2);
		bn_rand(b, BN_POS, BN_BITS_RELIC);
	...
	BENCH_BEGIN("bn_mod_2b") {
		bn_rand(a, BN_POS, BN_BITS_RELIC);
		BENCH_ADD(bn_mod_2b(c, a, BN_BITS_RELIC / 2));
	}
	BENCH_END;
	...
	BENCH_ONCE("bn_gen_prime", bn_gen_prime(a, BN_BITS_RELIC));

#if BN_GEN == BASIC || !defined(STRIP)
	BENCH_ONCE("bn_gen_prime_basic", bn_gen_prime_basic(a, BN_BITS_RELIC));
#endif

#if BN_GEN == SAFEP || !defined(STRIP)
	BENCH_ONCE("bn_gen_prime_safep", bn_gen_prime_safep(a, BN_BITS_RELIC));
#endif

#if BN_GEN == STRON || !defined(STRIP)
	BENCH_ONCE("bn_gen_prime_stron", bn_gen_prime_stron(a, BN_BITS_RELIC));
#endif
	...
	BENCH_BEGIN("bn_rec_win") {
		uint8_t win[BN_BITS_RELIC + 1];
		bn_rand(a, BN_POS, BN_BITS_RELIC);
		BENCH_ADD((len = BN_BITS_RELIC + 1, bn_rec_win(win, &len, a, 4)));
	}
	BENCH_END;
	...
	BENCH_BEGIN("bn_rec_slw") {
		uint8_t win[BN_BITS_RELIC + 1];
		bn_rand(a, BN_POS, BN_BITS_RELIC);
		BENCH_ADD((len = BN_BITS_RELIC + 1, bn_rec_slw(win, &len, a, 4)));
	}
	BENCH_END;

	BENCH_BEGIN("bn_rec_naf") {
		signed char naf[BN_BITS_RELIC + 1];
		int len;
		bn_rand(a, BN_POS, BN_BITS_RELIC);
		BENCH_ADD((len = BN_BITS_RELIC + 1, bn_rec_naf(naf, &len, a, 4)));
	}
	BENCH_END;
	...
	BENCH_BEGIN("bn_rec_reg") {
		...
		int len = BN_BITS_RELIC + 1;
		bn_rand(a, BN_POS, BN_BITS_RELIC);
		BENCH_ADD((len = BN_BITS_RELIC + 1, bn_rec_reg(naf, &len, a, BN_BITS_RELIC, 4)));
	}
	BENCH_END;

	BENCH_BEGIN("bn_rec_jsf") {
		signed char jsf[2 * (BN_BITS_RELIC + 1)];
		bn_rand(a, BN_POS, BN_BITS_RELIC);
		bn_rand(b, BN_POS, BN_BITS_RELIC);
		BENCH_ADD((len = 2 * (BN_BITS_RELIC + 1), bn_rec_jsf(jsf, &len, a, b)));
	}
	BENCH_END;
